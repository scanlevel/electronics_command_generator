...
if __name__ == "__main__":
    ...
    for device_name,action_classes in device_data.items():
        ...
        verb_to_sources = defaultdict(list)

        for action_class, actions in action_classes.items():
            for action_name in actions.keys():
                verb_list = action_data[action_class][action_name]['Verb']
                for v in verb_list:
                    verb_to_sources[v].append((action_class, action_name))

        for v, sources in verb_to_sources.items():
            verb_conjugated_form = run_pipeline(v)[v]
            verb_LCP = common_prefix_by_jamos_flexible(verb_conjugated_form)

            if verb_LCP not in device_verbs[device_name]:
                device_verbs[device_name][verb_LCP] = {
                    "forms": set(),
                    "sources": set()
                }

            device_verbs[device_name][verb_LCP]["forms"].update(verb_conjugated_form)
            device_verbs[device_name][verb_LCP]["sources"].update(sources)
    ...
    for lcp in common_verbs:
        ...
        for device_name, verb_map in device_verbs.items():
            if lcp in verb_map:
                entry = verb_map[lcp]
                all_forms.update(entry["forms"])
                all_sources.extend([
                    {
                        "device": device_name,
                        "action_class": ac,
                        "action_name": an
                    } for (ac, an) in entry["sources"]
                ])
                ...
                "source": [
                    {
                        ...
                        "action_class": ac,
                        "action_name": an
                    } for (ac, an) in data["sources"]
                ]
            } for lcp, data in device_verbs[device_name].items()
        }
    ...
    print(f"분류 끝 : {time.time() - start:.3f} 초")